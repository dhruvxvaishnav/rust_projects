...
pub fn handle_command(cmd: &str, store: &Store) -> String {
    ...
    match parts[0].to_uppercase().as_str() {
        ...
        "GET" if parts.len() == 2 => match db.get(parts[1]) {
        ...
        "TTL" if parts.len() == 2 => match db.get(parts[1]) {
        ...
        "RENAME" if parts.len() == 3 => match db.remove(parts[1]) {
            Some(entry) => {
                db.insert(parts[2].to_string(), entry);
                "+OK\r\n".to_string()
            }
